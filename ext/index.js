...
const callback = (mutationList, observe) => {
    if (check && window.location.pathname.match(POST_REGEX)) {
        ...
        if (b && !document.querySelector(".downloadimage")) {
            check = false
            itemDOM.addEventListener("click", getdata("home"))
            b.appendChild(itemDOM)
            timeout = setTimeout(() => {
                check = true
            }, 1000)
        }
            ...
            itemDOM.addEventListener("click", getdata("post"))
            c.appendChild(itemDOM)
            timeout = setTimeout(() => {
                check = true
            }, 1000)
        }
...
// document.querySelectorAll(".x1iyjqo2.x1tjbqro.x78zum5.x6s0dn4.xeud2gj")
// document.querySelectorAll("._aamj._acvz._acnc._acng")